...
function setupAdvancedForm() {
    ...
    // Validação especial para o checkbox de termos
    const termsCheckbox = document.getElementById('terms');
    if (termsCheckbox) {
        termsCheckbox.addEventListener('change', () => {
            const errorElement = termsCheckbox.closest('.form-group').querySelector('.form-error');
            if (errorElement && termsCheckbox.checked) {
                errorElement.classList.remove('show');
            }
        });
    }
    
    // Submissão do formulário - Compatível com Netlify + Redirecionamento
    form.addEventListener('submit', function(e) {
        ...
        // Se estiver em produção (Netlify), permitir envio nativo com redirecionamento
        if (window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
            // Deixar o Netlify processar e redirecionar automaticamente
            return; // Permitir envio nativo
        }
        
        // Para desenvolvimento local, simular redirecionamento
        e.preventDefault();
        
        setTimeout(() => {
            // Simular redirecionamento para página de agradecimento
...
function validateField(field) {
    ...
    // Validações específicas mais flexíveis
    if (field.hasAttribute('required') && !field.value.trim()) {
        ...
        // Telefone super flexível
        const cleanPhone = field.value.replace(/\D/g, '');
        
        // Aceitar telefones com 8 a 11 dígitos
        ...
        if (field.value.trim().length < 10) {
            isValid = false;
            errorMessage = 'Descreva seu caso com mais detalhes';
        }
    }
...
function validateForm() {
    ...
    if (!termsCheckbox.checked) {
        ...
        const errorElement = termsCheckbox.closest('.form-group').querySelector('.form-error');
        if (errorElement) {
            errorElement.textContent = 'Por favor, aceite os termos para continuar';
            errorElement.classList.add('show');
        }
    } else {
        // Remover erro se checkbox estiver marcado
        const errorElement = termsCheckbox.closest('.form-group').querySelector('.form-error');
        if (errorElement) {
            errorElement.classList.remove('show');
        }
